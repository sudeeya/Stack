...
Stack<T>::Stack(const T* data, size_t size)
    : capacity_(InitialCapacity), size_(size) {
  while (size_ > capacity_) capacity_ *= MemoryMultiplier;
  data_ = new T[capacity_];
...
void Stack<T>::push(T data) {
  if (size_ == capacity_) data_realloc();
  data_[size_] = data;
  ++size_;
}
...
Stack<bool>::Stack() : capacity_(InitialCapacity), size_(0) {
  ...
  for (size_t i = 0; i < byte_capacity; ++i) data_[i] = 0;
}

Stack<bool>::Stack(const bool* data, size_t size)
    : capacity_(InitialCapacity), size_(size) {
  while (size_ > capacity_) capacity_ *= MemoryMultiplier;
  size_t byte_capacity = get_byte_capacity();
  data_ = new unsigned char[byte_capacity];
...
Stack<bool>::Stack(const Stack<bool>& other)
  ...
  size_t byte_capacity = get_byte_capacity();
  data_ = new unsigned char[byte_capacity];
  for (size_t i = 0; i < byte_capacity; ++i) data_[i] = other.data_[i];
}

Stack<bool>::Stack(Stack<bool>&& other)
    : data_(other.data_), capacity_(other.capacity_), size_(other.size_) {
  other.data_ = nullptr;
}
...
size_t Stack<bool>::size() const { return size_; }

size_t Stack<bool>::capacity() const { return capacity_; }
...
bool Stack<bool>::operator==(const Stack<bool>& other) const {
  ...
  for (size_t i = 0; i < size_ / SizeOfChar + 1; ++i) {
    for (size_t j = 0; j < SizeOfChar; ++j) {
      if (((data_[i] >> (SizeOfChar - j - 1)) & 1) !=
          ((other.data_[i] >> (SizeOfChar - j - 1)) & 1))
        return false;
      ++data_index;
      if (data_index == size_) return true;
    }
  }
  return true;
}
...
size_t Stack<bool>::get_byte_capacity() const {
  if (capacity_ % SizeOfChar == 0) return capacity_ / SizeOfChar;
...
void Stack<bool>::data_realloc() {
  ...
  for (size_t i = 0; i < old_byte_capacity; ++i) tmp[i] = data_[i];
  capacity_ *= MemoryMultiplier;
  size_t new_byte_capacity = get_byte_capacity();
  data_ = new unsigned char[new_byte_capacity];
  for (size_t i = 0; i < new_byte_capacity; ++i) data_[i] = 0;
  for (size_t i = 0; i < old_byte_capacity; ++i) data_[i] = tmp[i];
  delete[] tmp;
}
